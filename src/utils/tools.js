  ...
  if(data){
      if(typeof data === 'object'){
          ...
          for(var sAttr in data){
              tempData.push(encodeURIComponent(sAttr)+'='+encodeURIComponent(data[sAttr]))
          }
          data = tempData.join('&');
      }
      // get请求拼接url
  ...
  oXhr.onreadystatechange = function() {
      ...
      if(oXhr.readyState === 4 && oXhr.status === 200){
          if(success && typeof success === 'function'){
              if(dataType){
                  switch(dataType){
                      case 'xml': return success(oXhr.responseXML);
                      case 'json': return success(JSON.parse(oXhr.responseText));
                      default: return success(oXhr.responseText);
                  }
              }else {
